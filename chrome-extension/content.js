(async () => {
  ...
  // ── Mode: create client list (triggered from Settings page) ─────────────────
...
// querySelectorAll does not cross shadow boundaries in Lit/React components.
...
async function runCreateClientList(appBaseUrl) {
  ...
  try {
    ...
    if (!companies || companies.length === 0) throw new Error('No hay empresas en la Lista de clientes. Agrega empresas en Settings primero.');
    ...
    setStatus('Error: ' + err.message);
    setProgress('Cerra esta pestana y revisa la configuracion en ProspectOS.');
    console.error('[ProspectOS create_client_list]', err);
  }
}

// Called on each page load while state exists (navigating one search per company)
async function resumeClientListFlow(state) {
  ...
  while (Date.now() < deadline) {
    ...
    if (/no result|sin resultado|0 result/i.test(document.body ? document.body.innerText : '')) break;
    await new Promise(r => setTimeout(r, 600));
  }

  const newResolved = [...resolved];
  if (companyId) {
    setProgress('OK ' + displayName + ' -> ' + companyId);
  ...
  if (nextIndex < toResolve.length) {
    const nextState = Object.assign({}, state, { resolved: newResolved, currentIndex: nextIndex });
...
async function doCreateList(resolved, appBaseUrl, { setStatus, setProgress }, totalCount) {
  if (resolved.length === 0) {
    ...
    setProgress('Intenta agregar las URLs de LinkedIn de cada empresa en Settings.');
    return;
  }
  ...
  const jsessionRaw = document.cookie.split(';')
    ...
    ?.[1]?.replace(/"/g, '') || '';
  const csrfToken = jsessionRaw.startsWith('ajax:') ? jsessionRaw : 'ajax:' + jsessionRaw;
  if (!csrfToken || csrfToken === 'ajax:') throw new Error('No se encontro el CSRF token.');
