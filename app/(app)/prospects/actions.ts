...
export async function getAllProspects() {
  const { data, error } = await supabase
    .from("prospects")
    .select("id, full_name, job_title, company_name, company_domain, linkedin_url, connection_degree, email, icp_score, is_premium, status, started_role_months, highlights, created_at, campaign_id, campaigns(week_label, rep_name, industry)")
  ...
  return (data ?? []) as unknown as {
    ...
    email: string | null; icp_score: number; is_premium: boolean; status: string
